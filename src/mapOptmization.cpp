...
Eigen::Affine3d Lidar2Camera = Eigen::Affine3d::Identity();
Eigen::Matrix3d rotation_matrix2;
bool forceSaveKey = false;// 保存tag的时候强行保存一个关键帧
...
/////variables corresponding to tag-based loop detected //////////////
    ...
    tag tagTemp{};
    std::vector <tag> tag_vector;//    std::vector<tag>::reverse_iterator tag_vector_iter;
...
//////////////////////////////////////////////////////////////////
    ...
    mapOptimization() {
        ...
        subTag = nh.subscribe<zapriltag_ros::TagsDetection_msg>("TagsDetected", 1000, &mapOptimization::tagCallback,
                ...
                if( id_temp != tagTemp.id &&tagTemp.time_stamp - pre_tag.time_stamp > tag_time_shift){
                  ...
                  //            << std::endl;
                   ...
                   //           << std::endl;
                    ...
                    posePre = cloudKeyPoses6D->points[pre_tag.correspondingRobotPoseID];
                 ...
                 //   std::cout << "lidarPre2tag:" << lidarPre2tag.position.x << " " << lidarPre2tag.position.y << " "
                   //           << lidarPre2tag.position.z << std::endl;
                    ...
                    tf::poseMsgToEigen(lidarCur2tag, rc2tag);// convert pose_msg to eigen matrix
                    tf::poseMsgToEigen(lidarPre2tag, rp2tag);
                 ...
                 //   std::cout << "rp2tag_temp:" << rp2tag_temp.matrix() << std::endl;
                  ...
                  //  std::cout << "posePre:" << p1.matrix() << std::endl;
                    ...
                    loopTagQueue.push_back(
                            make_pair(pre_tag.correspondingRobotPoseID, tagTemp.correspondingRobotPoseID));
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void saveKeyFramesAndFactor() {
        if (saveFrame() == false && forceSaveKey == false)
            return;
...
int main(int argc, char **argv) {
    ...
    rotation_matrix2 << 0, 0, 1, -1, 0, 0, 0, -1, 0;
    Lidar2Camera.translation() = Eigen::Vector3d(0.011, 0.033, 0.168);
    Lidar2Camera.linear() = rotation_matrix2;
